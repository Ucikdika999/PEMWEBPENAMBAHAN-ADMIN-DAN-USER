...
if (isset($_POST['tambah'])) {
    ...
    $sql = "INSERT INTO destinasi
                (nama_wisata, lokasi, harga, jenis_wisata, status_buka,
                 jam_buka, jam_tutup, cuaca, musim_terbaik, deskripsi, foto_url, fasilitas)
            VALUES
                ('$nama','$lokasi','$harga','$jenis','$status',
                 '$jam_buka','$jam_tutup','$cuaca','$musim','$deskripsi','$foto_url','$fasilitas')";

    if (mysqli_query($koneksi, $sql)) {
        $pesan_ok = "Destinasi berhasil ditambahkan!";
    } else {
        $pesan_err = "Gagal menambahkan: " . mysqli_error($koneksi);
    }
}
...
if (isset($_POST['edit'])) {
    ...
    $sql = "UPDATE destinasi SET
                lokasi='$lokasi', harga='$harga', jenis_wisata='$jenis',
                status_buka='$status', jam_buka='$jam_buka', jam_tutup='$jam_tutup',
                cuaca='$cuaca', musim_terbaik='$musim', deskripsi='$deskripsi',
                foto_url='$foto_url', fasilitas='$fasilitas'
            WHERE id_wisata='$id'";

    if (mysqli_query($koneksi, $sql)) {
        $pesan_ok = "Data berhasil diperbarui!";
    } else {
        $pesan_err = "Gagal memperbarui: " . mysqli_error($koneksi);
    }
}
...
.btn-primary-c { background:var(--accent); color:#fff; border:none; border-radius:9px; padding:8px 18px; font-size:.83rem; font-weight:600; font-family:inherit; cursor:pointer; display:inline-flex; align-items:center; gap:7px; transition:.18s; white-space:nowrap; text-decoration:none; }
...
.badge-cuaca { padding:3px 9px; border-radius:20px; font-size:.72rem; font-weight:600; display:inline-flex; align-items:center; gap:4px; white-space:nowrap; }
.cuaca-cerah   { background:#fef9c3; color:#854d0e; }
.cuaca-berawan { background:#e2e8f0; color:#475569; }
.cuaca-hujan   { background:#dbeafe; color:#1e40af; }
...
.modal-box { background:#fff; border-radius:18px; padding:28px; width:100%; max-width:640px; animation:mIn .2s ease; margin:auto; }
...
.cuaca-row { display:flex; align-items:center; gap:8px; }
.cuaca-row select { flex:1; }
.cuaca-info { margin-top:6px; font-size:.78rem; color:var(--text-muted); min-height:20px; display:flex; align-items:center; gap:4px; }
...
.btn-save     { background:var(--accent); color:#fff; border:none; border-radius:9px; padding:9px 22px; font-family:inherit; font-size:.83rem; font-weight:600; cursor:pointer; display:inline-flex; align-items:center; gap:6px; }
.btn-save-warn { background:var(--warning); }
...
<main class="main">
    ...
    <?php if ($pesan_ok):  ?><div class="alert-ok"><i class="fas fa-check-circle"></i> <?= htmlspecialchars($pesan_ok) ?></div><?php endif; ?>
    <?php if ($pesan_err): ?><div class="alert-err"><i class="fas fa-exclamation-circle"></i> <?= htmlspecialchars($pesan_err) ?></div><?php endif; ?>
    ...
    <form method="GET" class="toolbar">
        ...
        <button type="button" class="btn-primary-c"
    ...
    <div class="table-card">
        ...
        <table>
            ...
            function cuacaBadge($c) {
                $c = strtolower($c ?? '');
                if (str_contains($c,'hujan lebat'))  return ['cls'=>'cuaca-hujan',   'icon'=>'🌧️'];
                if (str_contains($c,'hujan ringan')) return ['cls'=>'cuaca-hujan',   'icon'=>'🌦️'];
                if (str_contains($c,'berawan'))      return ['cls'=>'cuaca-berawan', 'icon'=>'⛅'];
                return                                      ['cls'=>'cuaca-cerah',   'icon'=>'☀️'];
            }
            ...
            <tr>
                ...
                <td>
                    <?php if (!empty($d['foto_url'])): ?>
                        <img src="<?= htmlspecialchars($d['foto_url']) ?>" class="foto-thumb"
                             onerror="this.replaceWith(Object.assign(document.createElement('div'),{className:'no-foto',innerHTML:'<i class=\'fas fa-image\'></i>'}))">
                    <?php else: ?>
                ...
                <td style="font-weight:700;">Rp<?= number_format($d['harga'], 0, ',', '.') ?></td>
                ...
                <td>
                    <span class="badge-cuaca <?= $cb['cls'] ?>">
                        <?= $cb['icon'] ?> <?= htmlspecialchars(ucfirst($d['cuaca'] ?? '-')) ?>
                    </span>
                </td>
                <td>
        ...
        <div class="pagination-wrap">
            <div class="pag-info">
                Menampilkan <?= min($offset + 1, $total) ?>–<?= min($offset + $per_page, $total) ?> dari <?= $total ?> data
            </div>
            <div class="pag-btns">
                <?php if ($page > 1): ?>
                    <a href="?page=<?= $page-1 ?>&q=<?= urlencode($search) ?>&kategori=<?= urlencode($filter_kategori) ?>" class="pag-btn">
                        <i class="fas fa-chevron-left" style="font-size:.7rem;"></i>
                    </a>
                <?php endif; ?>
                <?php for ($p = 1; $p <= $total_pages; $p++): ?>
                    <a href="?page=<?= $p ?>&q=<?= urlencode($search) ?>&kategori=<?= urlencode($filter_kategori) ?>"
...
<div class="modal-box">
    ...
    <form method="POST">
        <div class="form-row">
            ...
            <div class="form-group form-full">
                <label>Lokasi</label>
                <input type="text" name="lokasi" id="tambahLokasi" placeholder="contoh: Wonosobo, Jawa Tengah">
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" name="harga" placeholder="25000" min="0" required>
            </div>
            ...
            <div class="form-group form-full">
                <label>Kondisi Cuaca
                    <span style="font-weight:400;color:#94a3b8;font-size:.72rem;margin-left:4px;">(isi lokasi lalu klik Auto)</span>
                </label>
                <div class="cuaca-row">
                    <select name="cuaca" id="cuacaTambah">
                        <option>Cerah</option>
                    ...
                    <button type="button" class="btn-primary-c btn-auto" id="btnAutoTambah"
                        onclick="fetchCuaca('Tambah')">
                        <i class="fas fa-cloud-sun"></i> Auto
                    </button>
                </div>
                <div class="cuaca-info" id="cuacaInfoTambah"></div>
            </div>
        ...
        <div class="modal-footer">
            <button type="button" class="btn-cancel"
                onclick="document.getElementById('modalTambah').classList.remove('show')">Batal</button>
...
<div class="modal-box">
    ...
    <form method="POST" id="formEdit">
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                <label>Harga Tiket (Rp)</label>
                <input type="number" name="harga" id="editHarga" min="0" required>
            </div>
            ...
            <div class="form-group form-full">
                ...
                <div class="cuaca-row">
                    ...
                    <button type="button" class="btn-primary-c btn-auto" id="btnAutoEdit"
                        onclick="fetchCuaca('Edit')">
                        <i class="fas fa-cloud-sun"></i> Auto
                    </button>
                </div>
                <div class="cuaca-info" id="cuacaInfoEdit"></div>
            </div>
        ...
        <div class="modal-footer">
            <button type="button" class="btn-cancel"
                onclick="document.getElementById('modalEdit').classList.remove('show')">Batal</button>
            <button type="submit" name="edit" class="btn-save btn-save-warn">
...
// ── Fetch cuaca otomatis via OpenWeatherMap ──────────────────────────────
function fetchCuaca(suffix) {
    const lokasiEl  = suffix === 'Tambah'
        ? document.getElementById('tambahLokasi')
    ...
    const btnEl     = document.getElementById('btnAuto' + suffix);
    const lokasi    = lokasiEl ? lokasiEl.value.trim() : '';

    if (!lokasi) {
        infoEl.innerHTML = '<span style="color:#ef4444;"><i class="fas fa-exclamation-triangle"></i> Isi kolom Lokasi terlebih dahulu.</span>';
        return;
    }

    // Loading state
    btnEl.disabled = true;
    btnEl.innerHTML = '<span class="spinner"></span> Memuat...';
    infoEl.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Mengambil data cuaca real-time...';

    fetch('get_cuaca.php?lokasi=' + encodeURIComponent(lokasi))
        .then(r => r.json())
        .then(data => {
            btnEl.disabled = false;
            btnEl.innerHTML = '<i class="fas fa-cloud-sun"></i> Auto';

            if (data.error) {
                infoEl.innerHTML = `<span style="color:#ef4444;"><i class="fas fa-exclamation-triangle"></i> ${data.error}</span>`;
                return;
            }
            ...
            const iconHtml = data.icon
                ? `<img src="${data.icon}" style="width:22px;height:22px;vertical-align:middle;">`
                : '';
            infoEl.innerHTML =
                `${iconHtml} <strong>${data.label}</strong> — ${data.desc}, ` +
        ...
        .catch(() => {
            btnEl.disabled = false;
            btnEl.innerHTML = '<i class="fas fa-cloud-sun"></i> Auto';
            infoEl.innerHTML = '<span style="color:#ef4444;"><i class="fas fa-exclamation-triangle"></i> Gagal menghubungi server. Periksa koneksi.</span>';
        });
}
...
function openEditModal(d) {
    ...
    // Cuaca
    const cuacaVal = d.cuaca || 'Cerah';
    const cuacaSel = document.getElementById('editCuaca');
    cuacaSel.value = cuacaVal;
    // Kalau tidak cocok (nilai lama), pilih default
    if (!cuacaSel.value) cuacaSel.value = 'Cerah';
    ...
    const elFas = document.getElementById('fasilitasEdit');
    elFas.dataset.checked = JSON.stringify(fas);
